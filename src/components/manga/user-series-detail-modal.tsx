...
export function UserSeriesDetailModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl sm:max-w-5xl md:max-w-5xl lg:max-w-5xl w-[95vw] max-h-[92vh] flex flex-col p-0 overflow-hidden bg-[#090D18]/98 border-white/15 text-white backdrop-blur-3xl shadow-2xl rounded-3xl">
        ...
        <div className="relative p-6 pb-4 border-b border-white/10 bg-gradient-to-r from-purple-950/40 via-[#0B1020] to-cyan-950/40 overflow-hidden">
          ...
          <DialogHeader className="p-0 border-0 bg-transparent space-y-4">
            <div className="flex flex-col sm:flex-row gap-5 items-start">
              {/* Series Cover Poster */}
              <div className="relative aspect-[2/3] w-24 sm:w-28 overflow-hidden rounded-2xl bg-black shrink-0 border border-white/20 shadow-2xl">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={getCoverUrl(mainCover)}
                  alt={series.title}
                ...
                />
                <div className="absolute top-1.5 left-1.5 rounded-md bg-black/80 px-1.5 py-0.5 text-[9px] font-black text-cyan-300 border border-white/10">
                  {series.publisher}
                </div>
              </div>
              ...
              {filteredVolumes.map((vol) => {
                ...
                return (
                  <div
                    ...
                    className={`group relative flex flex-col rounded-2xl overflow-hidden p-2.5 border transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${
                      isRead
                        ? 'bg-[#150F26] border-purple-500/40 shadow-sm shadow-purple-500/10 hover:border-purple-400'
                        : isOwned
                        ? 'bg-[#0B1D19] border-emerald-500/40 shadow-sm shadow-emerald-500/10 hover:border-emerald-400'
                        : isWishlist
                        ? 'bg-[#220D16] border-rose-500/30 hover:border-rose-400'
                        : isOrdered
                        ? 'bg-[#0B1925] border-cyan-500/30 hover:border-cyan-400'
                        : 'bg-white/[0.03] border-white/10 opacity-60 hover:opacity-85'
                    }`}
                  >
                    {/* Volume Cover */}
                    <div className="relative aspect-[2/3] w-full overflow-hidden rounded-xl bg-black border border-white/10 mb-2 shadow-md">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img
                        ...
                        className={`h-full w-full object-cover group-hover:scale-105 transition-transform duration-300 ${isNone ? 'grayscale contrast-75' : ''}`}
                      />

                      {/* Volume Number Tag */}
                      <div className="absolute top-1.5 left-1.5 rounded-md bg-black/85 px-1.5 py-0.5 text-[9px] font-black text-white border border-white/20 shadow">
                        T.{vol.volumeNumber}
                      </div>

                      {/* Volume Rating if provided */}
                      {vol.userRating && (
                        <div className="absolute top-1.5 right-1.5 flex items-center gap-0.5 rounded-md bg-black/85 px-1.5 py-0.5 text-[9px] font-black text-amber-400 border border-amber-500/30 shadow">
